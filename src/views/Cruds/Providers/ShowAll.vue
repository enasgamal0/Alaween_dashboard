    ...
    <main>
        ...
        <div class="filter_form_wrapper">
          <form @submit.prevent="submitFilterForm">
            <div class="row justify-content-center align-items-center w-100">
              ...
              <!-- <base-select-input col="4" :optionsList="allMainProviders" :placeholder="$t('PLACEHOLDERS.main_section')"
                v-model="filterOptions.main_section" required /> -->

              <!-- <base-select-input col="4" :optionsList="allSubProviders" :placeholder="$t('PLACEHOLDERS.sub_section')"
      ...
      <div class="table_title_wrapper">
        <div class="title_text_wrapper">
          <h5>{{ $t("PLACEHOLDERS.providers_management") }}</h5>
          <button
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            ...
            >
              <template slot="title">
                <span>{{ $t("BUTTONS.social_data") }}</span>
              </template>
              <button class="btn_show" @click="socialItem(item)">
            ...
            >
              <template slot="title">
                <span>{{ $t("BUTTONS.work_hours") }}</span>
              </template>
              <button class="btn_show" @click="timeItem(item)">
...
export default {
  ...
  methods: {
      ...
      try {
        let res = await this.$axios({
          ...
          params: {
            ...
            category_id: this.filterOptions.category_id?.id,
    ...
    socialItem(item) {
      this.$router.push({ path: `/providers/provider-social/${item.id}` });
    },
    timeItem(item) {
      this.$router.push({ path: `/providers/provider-hours/${item.id}` });
    },
    // ===== End:: Edit
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `stores/${this.itemToDelete.id}`,
        });
